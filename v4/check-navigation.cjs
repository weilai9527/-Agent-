...
for(const file of ['app.js','edition.js','voice.js','navigation.js'])run(fs.readFileSync(file,'utf8'));
async function click(dataset){let stopped=false;const e={preventDefault(){},stopImmediatePropagation(){stopped=true},target:{closest(selector){if(selector==='[data-parent]')return dataset.parent?{dataset}:null;if(selector==='[data-action="start"]')return dataset.action==='start'?{dataset}:null;if(selector.includes('[data-voice]'))return{dataset};if(selector==='[data-action]'&&dataset.action)return{dataset};if(selector==='[data-v2]'&&dataset.v2)return{dataset};return null}}};for(const {fn} of [...listeners.click].sort((a,b)=>Number(!!b.capture)-Number(!!a.capture))){await fn(e);if(stopped)break}}
(async()=>{
 ...
 const parents={home:'login',phone:'home',portrait:'home',me:'home',custom:'phone',resume:'me',history:'me',analysis:'resume',report:'history'};
 for(const [page,parent] of Object.entries(parents)){context.location.hash='#'+page;run('render()');assert(app.innerHTML.includes('data-parent="'+parent+'"'));assert.equal((app.innerHTML.match(/data-parent=/g)||[]).length,1);await click({parent});assert.equal(context.location.hash,'#'+parent)}
 for(const source of ['phone','custom']){context.location.hash='#'+source;await click({action:'start',value:'简单'});run('render()');assert.equal(run("parentOf('interview')[0]"),source);await click({parent:source});assert.equal(context.location.hash,'#'+source)}
 console.log('PASS: all logical parent routes, single return control, both interview entry paths.');
